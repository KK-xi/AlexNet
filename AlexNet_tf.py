...
mnist = input_data.read_data_sets("D:/learning materials_gruaduated/studying/python/pytorch/mnist_dataset/", one_hot=True)


# 定义网络超参数
learning_rate = 0.001
training_iters = 10000  # 这里只迭代一万次,准确率不会很高
batch_size = 50 #每个batch的大小
display_step = 50 #每20步展示一下结果
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters: # 直到达到最大迭代次数
    ...
    # 计算测试精度
    print ("Testing Accuracy:", sess.run(accuracy, feed_dict={x: mnist.test.images[:10], y: mnist.test.labels[:10], keep_prob: 1.})) # 拿前10个来测试
